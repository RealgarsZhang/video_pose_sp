...
def swap(T, m, n): #Distinguish left & right
    A = T.data.cpu().numpy()
    A[:, [m, n], :, :] = A[:, [n, m], :, :]
    return Variable(torch.from_numpy(A))
...
# =============================================================================
    def set_input(self, input):
        ...
        temp = self.input_A_S.clone()
        temp.resize_(input_A_S.size())
        temp.copy_(input_A_S)
        self.input_A_S = temp
        
        temp = self.input_A_L.clone()
        temp.resize_(input_A_L.size())
        temp.copy_(input_A_L)
        self.input_A_L = temp
        
        temp = self.input_A_Attribute.clone()
        temp.resize_(input_A_Attribute.size())
        temp.copy_(input_A_Attribute)
        self.input_A_Attribute = temp
        
        temp = self.input_B_GAN.clone()
        temp.resize_(input_B_GAN.size())
        temp.copy_(input_B_GAN)
        self.input_B_GAN = temp
        
        temp = self.input_B_L1.clone()
        temp.resize_(input_B_L1.size())
        temp.copy_(input_B_L1)
        self.input_B_L1 = temp
        
        temp = self.input_B_Attribute_GAN.clone()
        temp.resize_(input_B_Attribute_GAN.size())
        temp.copy_(input_B_Attribute_GAN)
        self.input_B_Attribute_GAN = temp
        
        temp = self.input_B_Attribute_L1.clone()
        temp.resize_(input_B_Attribute_L1.size())
        temp.copy_(input_B_Attribute_L1)
        self.input_B_Attribute_L1 = temp
    ...
    def test(self):
        self.real_A = Variable(self.input_A, volatile=True).cpu()
        #self.real_A.weight = self.real_A['weight'].cpu()
        self.real_A_S = Variable(self.input_A_S, volatile=True).cpu()
        #self.real_A_S.weight = self.real_A_S['weight'].cpu()
        self.real_A_L = Variable(self.input_A_L, volatile=True).cpu()
        #self.real_A_L.weight = self.real_A_L['weight'].cpu()

        M = nn.Upsample(int(self.real_A.size(3)), mode='bilinear')
        #print(self.real_A)
        #print("Other")
        #print(self.netG)
        #print(self.netG(self.real_A.cpu())['GAN'])
